...
// src/handlers/getPeopleHandler.ts
var getPeopleHandler_exports = {};
__export(getPeopleHandler_exports, {
  getPeople: () => getPeople
});
module.exports = __toCommonJS(getPeopleHandler_exports);
...
// src/services/baseAPI.service.ts
var import_dotenv = __toESM(require_main());
import_dotenv.default.config();
var SWAPI_API = process.env.SWAPI_BASE_URL;
var instanceSWAPI = axios_default.create({
  baseURL: SWAPI_API
});

// src/models/species.ts
var Species = class {
  name;
  classification;
  designation;
...
var People = class {
  ...
  gender;
  species;
  constructor(peopleDTO, speciesData) {
...
// src/utilities/autoMapperProfiles.ts
var Mapper = class _Mapper {
  static mapPeopleAttributes(people) {
    return {
      nombre: people.name,
      ...
      especies: _Mapper.mapSpeciesArray(people.species)
    };
  }
  static mapSpecies(species) {
    return {
      nombre: species.name,
  ...
  static mapSpeciesArray(speciesArray) {
    return speciesArray.map((species) => _Mapper.mapSpecies(species));
  }
};
...
var getPeople = (event) => {
  ...
  return instanceSWAPI.get(endpeople).then((response) => {
    if (response.status !== 200) {
      throw new Error("Error al obtener los datos del personaje de SWAPI");
    }
    const peopleDTO = response.data;
    const speciesPromises = peopleDTO.species.map((speciesUrl) => axios_default.get(speciesUrl));
    return Promise.all(speciesPromises).then((speciesResponses) => {
      const speciesData = speciesResponses.map((speciesResponse) => new Species(speciesResponse.data));
      ...
      return {
        ...
        body: JSON.stringify({
          success: true,
          message: "Se obtuvo la API con \xE9xito.",
